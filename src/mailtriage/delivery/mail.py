...
def event_time(cfg: Config, ev: Event) -> str:
    """The localized "All day", or "09:00–09:30" (just "09:00" for a
    zero-length event) -- clock times read the same in every language."""
    if ev["all_day"]:
        return t(cfg, "all_day")
...
def _today_block(cfg: Config, events: list[Event], invites: dict[int, int]) -> str:
    if not events:
        return ""
    rows = ""
    for i, ev in enumerate(events[:MAX_EVENTS]):
        ...
        if i in invites:
            hint = html.escape(t(cfg, "invite_in_inbox", n=invites[i]))
            extra += f' &nbsp;·&nbsp; <span class="muted" style="color:{DIM};">{hint}</span>'
        rows += f"""
      <tr><td style="padding:0 0 10px 0;font:400 14px/1.5 {SANS};color:{INK};">
        <span class="muted" style="display:inline-block;min-width:96px;color:{DIM};font-variant-numeric:tabular-nums;">{html.escape(event_time(cfg, ev))}</span>{title}{extra}
      </td></tr>"""
    return _section(t(cfg, "today"), rows)


def _draft_block(cfg: Config, draft: str, has_full: bool = False) -> str:
    if not draft:
        return ""
    fuller = (
        f'<div style="font:400 12px/1.4 {SANS};color:{DIM};margin:6px 0 0 0;">{html.escape(t(cfg, "draft_full"))}</div>'
        if has_full
        else ""
    )
    ...
    return f"""
        <div class="muted" style="margin:10px 0 0 0;padding:10px 12px;border-left:2px solid {RULE};">
          <div style="font:700 11px/1 {SANS};letter-spacing:.08em;color:{DIM};text-transform:uppercase;">{html.escape(t(cfg, "draft_reply"))}</div>
          <p style="font:400 14px/1.5 {SANS};color:{DIM};margin:6px 0 0 0;white-space:pre-wrap;">{html.escape(draft)}</p>{fuller}
        </div>"""


# Internal keys, never rendered raw: digest_groups tags each section with
# one and the renderer looks it up in delivery.strings. Reply commands and
# tests key off these, so they stay English no matter the reader's language.
GROUPS = ("overdue", "today", "this_week", "later", "no_date")
...
def section_heading(cfg: Config, key: str) -> str:
    """One of digest_groups' section keys, in the reader's language.
    "needs_action:overdue" -> "Needs action · Overdue"."""
    name, _, group = key.partition(":")
    if not group:
        return t(cfg, name)
    return t(cfg, "section_group", section=t(cfg, name), group=t(cfg, group))


def digest_groups(triaged: list[Triaged], today: date) -> list[tuple[str, str, list[Triaged]]]:
    """The digest's sections in render order, as (kind, section key, items)
    with kind in action/carried/reading. Items are numbered #1.. in exactly
    this order by every renderer (HTML and --dry-run text), and a reply to the
    digest addresses them by that number -- so the order lives here, once.

    The middle element is a KEY, not a heading: `section_heading(cfg, key)`
    turns it into the reader's language. It stays English and stable so
    tests, --dry-run and reply handling never depend on `language`.

    needs_action splits into overdue / today / this_week / later / no_date
    ...
    if any(due_date(t) for t in needs_action):
        ...
        for g, items in buckets.items():
            if items:
                ...
                out.append(("action", f"needs_action:{g}", items))
    elif needs_action:
        out.append(("action", "needs_action", needs_action))
    if carried:
        out.append(("carried", "still_waiting", carried))
    if worth_reading:
        out.append(("reading", "worth_reading", worth_reading))
    return out
...
def _due_line(cfg: Config, it: Triaged) -> str:
    d = due_date(it)
    if d is None:
        return ""
    return f"""
        <div class="muted" style="font:400 13px/1.4 {SANS};color:{DIM};padding-top:4px;">{html.escape(t(cfg, "due", date=f"{d:%a} {d.day} {d:%b}"))} &nbsp;·&nbsp; <a href="{html.escape(calendar_link(it), quote=True)}" style="color:{DIM};">{html.escape(t(cfg, "add_to_calendar"))}</a></div>"""


def _rows(cfg: Config, items: list[Triaged], start: int) -> str:
    out = ""
    for n, it in enumerate(items, start):
      ...
      <tr><td style="padding:0 0 26px 0;">
        ...
        <div class="muted" style="font:400 13px/1.4 {SANS};color:{DIM};padding-top:4px;">{dot}{html.escape(it["sender"])} &nbsp;·&nbsp; {html.escape(it["account"])}</div>{_due_line(cfg, it)}
        <p style="font:400 15px/1.55 {SERIF};color:{INK};margin:8px 0 0 0;">{html.escape(it["note"])}</p>{_draft_block(cfg, it["draft"], bool(it.get("draft_full")))}
      </td></tr>"""
    return out
...
def _carried_rows(cfg: Config, items: list[Triaged], start: int, nag_after_days: int) -> str:
    out = ""
    for n, it in enumerate(items, start):
        dot = "&#9679; " if it["unread"] else ""
        ...
        badge = (
            f' <span style="font:700 11px/1 {SANS};letter-spacing:.06em;color:{PAPER};background:{INK};padding:3px 6px;border-radius:3px;text-transform:uppercase;">{html.escape(t(cfg, "still_open"))}</span>'
            if nag
            else ""
        )
      ...
      <tr><td style="padding:0 0 26px 0;">
        ...
        <div class="muted" style="font:{"700" if nag else "400"} 13px/1.4 {SANS};color:{DIM};padding-top:4px;">{dot}{html.escape(it["sender"])} &nbsp;·&nbsp; {html.escape(it["account"])} &nbsp;·&nbsp; {html.escape(t(cfg, "waiting", days=t(cfg, "days", n=days)))}</div>
      </td></tr>"""
    return out
...
def _carried_footer(cfg: Config) -> str:
    return f"""
    <tr><td class="muted" style="font:400 12px/1.5 {SANS};color:{DIM};padding-top:2px;">{html.escape(t(cfg, "carried_footer", label=cfg.label))}</td></tr>"""


def _noise_section(cfg: Config, items: list[Triaged]) -> str:
    ...
    for it in items:
        ...
        if not link.lower().startswith(("https://", "mailto:")):
            continue
        rows += f"""
        <div style="font:400 13px/1.6 {SANS};color:{DIM};">{html.escape(it["sender"])} &nbsp;·&nbsp; <a href="{html.escape(link, quote=True)}" style="color:{DIM};">{html.escape(t(cfg, "unsubscribe"))}</a></div>"""
    if not rows:
        return ""
    return f"""
    <tr><td class="muted" style="padding-top:26px;">
      <details><summary style="cursor:pointer;font:700 13px/1 {SANS};letter-spacing:.1em;color:{DIM};text-transform:uppercase;">{html.escape(t(cfg, "noise_this_week"))}</summary>
...
def email_html(
    ...
    sections, n = _today_block(cfg, events, invite_numbers(events, triaged, today)), 1
    ...
    for kind, key, items in digest_groups(triaged, today):
        heading = section_heading(cfg, key)
        if kind == "carried":
            sections += _section(heading, _carried_rows(cfg, items, n, cfg.nag_after_days)) + _carried_footer(cfg)
        else:
            sections += _section(heading, _rows(cfg, items, n))
        n += len(items)
    sections += _noise_section(cfg, [x for x in triaged if x["bucket"] == "noise"])
...
<tr><td align="center" style="padding:32px 16px 44px 16px;">
  <table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="max-width:560px;">
    <tr><td style="padding-bottom:26px;border-bottom:1px solid {RULE};">
      ...
      <div class="muted" style="font:400 13px/1 {SANS};color:{DIM};padding-top:9px;">{html.escape(t(cfg, "act_read", a=len(needs_action), r=len(worth_reading)))}</div>
    </td></tr>
    {sections}
    <tr><td class="muted" style="border-top:1px solid {RULE};padding-top:18px;font:400 12px/1.5 {SANS};color:{DIM};">{html.escape(t(cfg, "digest_footer"))} {html.escape(COMMANDS_HINT)}</td></tr>
  </table>
</td></tr></table></body></html>"""


def _week_open_rows(cfg: Config, items: list[WeekItem]) -> str:
    out = ""
...
def _week_extra_section(cfg: Config, heading: str, items: list[WeekItem]) -> str:
    if not items:
        return ""
    return f"""
...
def _week_account_block(cfg: Config, account: str, buckets: dict[str, list[WeekItem]]) -> str:
    replied, archived, open_items = buckets["replied"], buckets["archived"], buckets["open"]
    open_rows = (
        _week_open_rows(cfg, open_items)
        if open_items
        else f'<tr><td class="muted" style="font:400 13px/1.5 {SANS};color:{DIM};padding:2px 0;">{html.escape(t(cfg, "nothing_open"))}</td></tr>'
    )
    return f"""
    <tr><td style="padding:26px 0 4px 0;">
      <div style="font:700 13px/1 {SANS};letter-spacing:.08em;color:{INK};text-transform:uppercase;">{html.escape(account)}</div>
      <div class="muted" style="font:400 12px/1.4 {SANS};color:{DIM};padding-top:6px;">{html.escape(t(cfg, "count_replied", n=len(replied)))} &nbsp;·&nbsp; {html.escape(t(cfg, "count_archived", n=len(archived)))} &nbsp;·&nbsp; {html.escape(t(cfg, "count_open", n=len(open_items)))}</div>
    </td></tr>
    <tr><td style="padding-top:12px;"><table role="presentation" width="100%" cellpadding="0" cellspacing="0">{open_rows}</table></td></tr>
    {_week_extra_section(cfg, t(cfg, "replied"), replied)}
    {_week_extra_section(cfg, t(cfg, "archived"), archived)}"""


def _narrative_block(cfg: Config, narrative: dict[str, Any] | None) -> str:
    ...
    patterns = (
        f"""
    <tr><td class="muted" style="font:700 12px/1.4 {SANS};color:{DIM};padding-top:12px;">{html.escape(t(cfg, "patterns"))}</td></tr>
...
def _saved_block(cfg: Config, totals: dict[str, int] | None) -> str:
    ...
    <tr><td class="muted" style="font:400 13px/1.5 {SANS};color:{DIM};padding-top:20px;">{
        html.escape(_saved_text(cfg, totals))
    }</td></tr>"""


def _saved_text(cfg: Config, totals: dict[str, int]) -> str:
    """Each count is rendered by its own plural fragment first, so every
    language keeps its own agreement rules inside delivery/strings.py."""
    return t(
        cfg,
        "saved",
        messages=t(cfg, "msgs", n=totals["triaged"]),
        replies=t(cfg, "replies", n=totals["drafts"]),
        minutes=t(cfg, "minutes", n=totals["minutes"]),
    )
...
def weekly_html(
    ...
    blocks = (
        _narrative_block(cfg, narrative)
        + _saved_block(cfg, totals)
        + "".join(_week_account_block(cfg, account, buckets) for account, buckets in week["accounts"].items())
    )
    return f"""<!doctype html><html><head><meta charset="utf-8">
<meta name="viewport" content="width=device-width,initial-scale=1">
<meta name="color-scheme" content="light dark">
...
<tr><td align="center" style="padding:32px 16px 44px 16px;">
  <table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="max-width:560px;">
    <tr><td style="padding-bottom:26px;border-bottom:1px solid {RULE};">
      <div style="font:700 26px/1 {SERIF};color:{INK};">{html.escape(t(cfg, "your_week"))}</div>
      <div class="muted" style="font:400 13px/1 {SANS};color:{DIM};padding-top:9px;">{html.escape(t(cfg, "week_handled", n=handled))}{" · " + html.escape(t(cfg, "week_done", n=done_count)) if done_count else ""} · {html.escape(t(cfg, "week_open", n=still_open))}</div>
    </td></tr>
    {blocks}
    <tr><td class="muted" style="border-top:1px solid {RULE};padding-top:18px;font:400 12px/1.5 {SANS};color:{DIM};">{html.escape(t(cfg, "weekly_footer", label=cfg.label))}</td></tr>
  </table>
</td></tr></table></body></html>"""
